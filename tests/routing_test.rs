...
fn test_bellman_ford_negative_cycle() {
    ...
    assert!(!routes.is_empty(), "Bellman-Ford failed to find negative cycle");
    ...
    let base_in_cycle = best_route.legs.iter().any(|leg| leg.token_in == best_route.base_token);
...
fn test_no_arbitrage_found_in_efficient_market() {
    ...
    let routes = router.find_arbitrage_routes();
    assert!(routes.is_empty(), "Found arbitrage in an efficient market (fees should prevent it)");
}
