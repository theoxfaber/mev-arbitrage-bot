...
fn test_evm_simulator_binary_search() {
    ...
    let pepe: Address = "0x6982508145454Ce325dDbE47a25d4ec3d2311933".parse().unwrap();
    ...
    let result = simulator.simulate(&route).expect("Simulation should succeed");
    
    // Profit must be strictly positive for this mispriced triangle
    assert!(result.gross_profit > U256::ZERO, "Expected positive gross profit");
    
    // The optimal loan size shouldn't be 0 or the max bounds
    assert!(result.optimal_loan_size > U256::from(10u64.pow(16)), "Optimal loan too small"); // > 0.01 WETH
    assert!(result.optimal_loan_size < U256::from(100u64) * U256::from(10u64.pow(18)), "Optimal loan too large");
    
    // Gas should be estimated reasonably for a 3-hop swap (base 50k + 3 * 80k)
    assert!(result.gas_used > 150_000 && result.gas_used < 500_000, "Gas estimation looks wrong");
}
