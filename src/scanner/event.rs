...
impl EventScanner {
    ...
    pub async fn process_log(&self, log_address: Address, topics: Vec<B256>, data: Vec<u8>) {
        ...
        if event_signature == B256::from_slice(&hex::decode("c42079f94a6350d7e6235f29174924f928cc2ac818eb64fed8004e115fbcca67").unwrap()) {
            self.handle_v3_swap(log_address, &topics, &data).await;
        } else if event_signature == B256::from_slice(&hex::decode("d78ad95fa46c994b6551d0da85fc275fe613ce37657fb8d5e3d130840159d822").unwrap()) {
